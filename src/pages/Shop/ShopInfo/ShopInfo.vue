  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item" v-for="(support, index) in info.supports" :key="index" :class="supportsClass[support.type]">
...
<script>
  ...
  export default {
    ...
    /*
    当直接在此路由刷新时，由于数据是异步请求，所以刷新后显示的是初始化数据，此时info为空，不能去创建BScroll对象，而需要去监视
    info的变化，等到异步请求获取数据变化后，在监视器中创建BScroll对像
     */
    watch:{
      info(){
        this.$nextTick(()=> {
    ...
    methods:{
      _initScroll(){
        this.$nextTick(()=>{
          //动态计算picsUl横向图片ul宽度
          /*
          ulWidth=(liWidth+space)*liCount-space
           */
